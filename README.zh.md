...
**适用场景：执行 `python3 ~/skills/.../token-stats.py setup` 时提示文件不存在。**

**原因 1：ClawHub 检测到 OpenClaw 工作区** → 技能被安装到 `~/.openclaw/workspace/skills/agent-usage-stats/`

**原因 2：ClawHub 把技能装到了执行 `clawhub install` 时所在目录的 `skills/` 文件夹下。**
