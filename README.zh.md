...
如果你用 [Hermes Agent](https://github.com/nousresearch/hermes-agent)，可以装 skill 配置：

```bash
cd ~/.hermes && clawhub install agent-usage-stats
```

> ⚠️ `clawhub install` 会装到**当前目录下的 `skills/`**。习惯上 Hermes 的技能目录在 `~/.hermes/skills/`，所以先 `cd ~/.hermes` 再装。

---

### 可选：VS Code 任务

把项目里的 `.vscode/tasks.json` 复制到你的项目目录，然后用 `Cmd+Shift+P` → Tasks: Run Task 选择：
...
# 如果装了 ClawHub 版本
cd ~/.hermes && clawhub uninstall agent-usage-stats
