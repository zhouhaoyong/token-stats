...
每 5 秒自动刷新（可自定义间隔），Ctrl+C 停止后输出汇总表。
...
#### ❓ `--compare` 显示「两个时间段均无数据」

**可能原因：** 两个时间段内都没有会话记录。先用 `--today` 确认当前是否有数据。

### 统计范围说明

> ⚠️ `token-stats` **只能统计本机的 Agent 数据**。
>
> - 如果你在 PC A 上跑 Hermes、在 PC B 上跑 Claude Code，各自的数据分别存在各自的电脑上
> - `token-stats` 读的是本机磁盘上的数据文件，不是从云端拉的
> - 要看另一台机器的统计，需要在那台机器上也装上 `token-stats`
> - 所有统计都是基于你选择的某个 Agent 来查询的，不是所有 Agent 的总和
